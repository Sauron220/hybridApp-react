...
class sliderRuler {
  ...
  dreawCanvas() {
    ...
    for (let i = Math.round(startValue / precision * 10) / 10; i <= endValue / precision; i++) {
      ...
      context.moveTo(origin.x + (i - startValue / precision) * divide, 0);
      context.lineTo(origin.x + divide, 0)
      ...
      if (i % 10 === 0) {
        ...
        context.fillText((Math.round(i / 10) / (derivative / 10)).toLocaleString(), origin.x + (i - startValue / precision) * divide, heightDecimal);
      }
      context.closePath();
    }
  }
